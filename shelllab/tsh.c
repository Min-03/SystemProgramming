...
{   
    ...
    sigprocmask(SIG_BLOCK, &mask_chld, &chld_prev);
    if ((pid = fork()) == 0) {
        sigprocmask(SIG_SETMASK, &chld_prev, NULL); 
        setpgid(0, 0);
...
//given argv in do_bgfg, return jid
int getjid(char **argv) {
    char *tgt = argv[1];
    int len = strlen(tgt);
    int jid = 0;
