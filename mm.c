...
int mm_init(void) 
{
    ...
    char *startp;
    if ((startp = mem_sbrk(4*WSIZE)) == (void *)-1)
	return -1;
    PUT(startp, 0);                        /* alignment padding */
    PUT(startp+WSIZE, PACK(OVERHEAD, 1));  /* prologue header */ 
    PUT(startp+DSIZE, PACK(OVERHEAD, 1));  /* prologue footer */ 
    PUT(startp+WSIZE+DSIZE, PACK(0, 1));   /* epilogue header */
...
void *mm_malloc(size_t size) 
{
    ...
    bp = extend_heap(MAX(CHUNKSIZE, realSize) / WSIZE);
...
static void *find_fit(size_t asize)
{
    ...
    for (rover = heap_listp; rover != NULL && rover < oldrover; rover = NEXT_BLKP_EX(rover))
	if (asize <= GET_SIZE(HDRP(rover)))
	    return rover;
    ...
    for (bp = heap_listp; bp != NULL; bp = NEXT_BLKP_EX(bp)) {
...
static void insertblock(void *bp) {
    ...
    PUT_ADDR(PRVP(tmp), bp);
    PUT_ADDR(PRVP(bp), NULL);
}

static void deleteblock(void *bp) {
    if (bp == heap_listp) heap_listp = NULL;
    void *nxt = NEXT_BLKP_EX(bp);
    void *prv = PREV_BLKP_EX(bp);
    if (nxt != NULL) {
        PUT_ADDR(PRVP(nxt), prv);
        if (heap_listp == NULL) {
            heap_listp = nxt;
        }
    }
